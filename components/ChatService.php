 ...
 * Handles both system queries and general educational guidance with ChatGPT-style flexibility
 */
class ChatService
{
    /**
     * Generate AI response based on user message - with full error handling
     */
    public function generateResponse($message, $context = [], $user = null, $assessmentId = null)
    {
        try {
            ...
            // Quick greeting override for short chat starts
            if (strpos($messageLower, 'hey') !== false || strpos($messageLower, 'hi') !== false || strpos($messageLower, 'hello') !== false) {
                return $this->greetingResponse($user);
            }

            // Check for greeting(s) in general text
            if ($this->isGreeting($messageLower)) {
                return $this->greetingResponse($user);
            }

            // Check for system statistics queries or "list" commands
            if ($this->containsKeyword($messageLower, ['student', 'list', 'how many', 'count', 'total', 'available']) || 
            ...
            if ($this->containsKeyword($messageLower, ['grade', 'level', 'score', 'scale', 'grading', 'assessment level', 'tp e24'])) {
            ...
            Yii::error('ChatService Error: ' . $e->getMessage() . '\n' . $e->getTraceAsString());
    ...
    private function startsWithKeyword($message, $keywords)
    {
        ...
        foreach ($keywords as $keyword) {
            if (!empty($keyword) && strpos($message, $keyword) === 0) {
                return true;
            }
        }
        return false;
    ...
    {
        $name = 'there';
        if ($user && is_object($user) && isset($user->name)) {
            $name = $user->name ?: 'Supervisor';
        }
        $greetings = [
            "Hello {$name}! 👋 I'm your TP Assessment Assistant. I'm here to help with grading, competency standards, assessment guidance, and answer any questions. What can I help you with?",
            "Welcome {$name}! 🎓 I can assist you with TP E24 grading scales, competency rubrics, assessment techniques, system info, and general knowledge. What's your question?",
            "Hi {$name}! 💡 Ready to help with your assessments and system queries. Feel free to ask about grades, standards, statistics, or any topic.",
        ];
    ...
    private function handleStatsQuery($message, $user)
    {
        try {
            ...
            // Student counts - "how many students", "total students", "students registered", "list all students"
            $isStudentQuery = (strpos($message, 'student') !== false || strpos($message, 'students') !== false);
            $isStudentCountRequest = (strpos($message, 'how many') !== false || strpos($message, 'total') !== false || 
                 strpos($message, 'count') !== false || strpos($message, 'registered') !== false || 
                 strpos($message, 'list') !== false || strpos($message, 'available') !== false || 
                 trim($message) === 'student' || trim($message) === 'students');

            if ($isStudentQuery && $isStudentCountRequest) {
                ...
                $yourStudents = 0;
                if ($user && is_object($user) && isset($user->user_id)) {
                    $yourStudents = Assessment::find()
                ...
                // If asking to list students
                if (strpos($message, 'list') !== false) {
                    $students = Assessment::find()
                        ->select(['student_reg_no'])
                        ->distinct()
                        ->orderBy('student_reg_no')
                        ->column();
                    
                    $response = "📚 **Students in System:**\n\n";
                    if (empty($students)) {
                        $response .= "No students have assessments yet.\n\nStart by creating an assessment for a student!";
                    } else {
                        foreach ($students as $student) {
                            $response .= "• $student\n";
                        }
                        $response .= "\n**Total:** " . count($students) . " different students";
                    }
                } else {
                    $response = "📊 **Student Statistics:**\n\n";
                    $response .= "• **Total students assessed (system-wide):** " . ($totalStudents ?: 0) . "\n";
                    if ($user && is_object($user) && isset($user->role_id) && $user->role_id == 1) {
            ...
            if ((strpos($message, 'assessment') !== false) && 
                ...
                $yourAssessments = 0;
                if ($user && is_object($user) && isset($user->user_id)) {
                    $yourAssessments = Assessment::find()
                        ->where(['examiner_user_id' => $user->user_id])
                        ->count();
                }
            ...
            // User counts - "how many users", "total users"
            if ((strpos($message, 'user') !== false) && 
                (strpos($message, 'how many') !== false || strpos($message, 'total') !== false || strpos($message, 'count') !== false)) {
                
                $totalUsers = Users::find()->count();
                $activeUsers = Users::find()->where(['status' => 'active'])->count();
                $response = "👥 **User Statistics:**\n\n";
                $response .= "• **Total users (system-wide):** " . ($totalUsers ?: 0) . "\n";
                $response .= "• **Active users:** " . ($activeUsers ?: 0) . "\n";
                $response .= "\nUsers include supervisors, examiners, and administrators in the TP Assessment System.";
                return $response;
            }

            return $response;
        } catch (\Exception $e) {
            Yii::error('Stats Query Error: ' . $e->getMessage());
            return "I can help with statistics! Ask about students, assessments, schools, or competencies in the system.";
        }
    }
    ...
    {
        try {
            $competencies = CompetenceArea::find()->limit(6)->all();
            ...
            foreach ($competencies as $comp) {
                if (isset($comp->competence_name)) {
                    $response .= "• **{$comp->competence_name}**\n";
                    if (isset($comp->description) && !empty($comp->description)) {
            ...
            Yii::error('Competency Error: ' . $e->getMessage());
            return "I can help with competency standards! Ask about specific competencies or assessment rubrics.";
        }
    }
    ...
    /**
     * Answer general knowledge questions on ANY topic (ChatGPT-style) with full robustness
     */
    private function generalKnowledgeResponse($message, $user)
    {
        try {
            ...
            // === YES/NO QUESTIONS ===
            if (strpos($messageLower, '?') !== false && !$this->startsWithKeyword($messageLower, ['how', 'why', 'what', 'when', 'where', 'who', 'which', 'whose', 'whom'])) {
                return $this->generateYesNoAnswer($messageLower);
            }

            // === WHO QUESTIONS ===
            if ($this->startsWithKeyword($messageLower, ['who is', 'who are', 'who was', 'who were'])) {
                return $this->generateWhoAnswer($messageLower);
            }

            // === WHEN QUESTIONS ===
            if ($this->startsWithKeyword($messageLower, ['when is', 'when are', 'when was', 'when were', 'when do', 'when did'])) {
                return $this->generateWhenAnswer($messageLower);
            }

            // === WHERE QUESTIONS ===
            if ($this->startsWithKeyword($messageLower, ['where is', 'where are', 'where was', 'where were'])) {
                return $this->generateWhereAnswer($messageLower);
            }

            // === HOW MUCH QUESTIONS ===
            if ($this->startsWithKeyword($messageLower, ['how much', 'how many'])) {
                return $this->generateHowMuchAnswer($messageLower);
            }

            // === OTHER QUESTIONS ===
            if (strpos($messageLower, '?') !== false) {
                return $this->generateOtherQuestionAnswer($messageLower);
            }
            ...
            Yii::error('General Knowledge Error: ' . $e->getMessage());
            return "I'm here to help! Feel free to ask me about:\n• Assessment and grading\n• System statistics\n• General knowledge\n• Teaching tips\n\nWhat can I help you with?";
        }
    }
    ...
    {
        $responses = [
            "That's a great question! 🤔\n\n✔️ I can help with assessment workflows, grading (BE/AE/ME/EE), competency standards, and general guidance.\n✔️ System stats (students/assessments/schools).\n✔️ Teaching tips and evidence collection.\n\nAsk me anything specific and I'll respond clearly.",
    ...
    /**
     * Generate answer for "Who..." questions
     */
    private function generateWhoAnswer($message)
    {
        $topic = str_replace(['who is', 'who are', 'who was', 'who were'], '', $message);
        $topic = trim($topic, '?');

        $responses = [
            "Regarding $topic, they are important figures in their field. In educational contexts, understanding key people helps with learning and assessment. Would you like me to explain their contributions to teaching or standards?",
            "$topic is a notable person or entity. In the world of education, they have influenced teaching methods, curriculum development, and assessment practices. What aspect interests you most?",
            "That's a question about $topic. As an assessment assistant, I can tell you that in educational settings, such individuals often contribute to best practices and professional development. Need more details on their impact?",
            "About $topic - they play a significant role in their area. In education, understanding influential people helps with implementing effective teaching strategies and assessment techniques. How can I help you learn more?",
        ];
     ...
     */
    private function generateWhenAnswer($message)
    {
        $topic = str_replace(['when is', 'when are', 'when was', 'when were', 'when do', 'when did'], '', $message);
        $topic = trim($topic, '?');

        $responses = [
            "Regarding $topic, timing is important in educational contexts. Generally, these things happen at optimal moments for learning and assessment. Would you like guidance on scheduling or best practices?",
            "About $topic - the timing depends on various factors. In education, we consider developmental stages, curriculum pacing, and assessment windows. What specific timing are you asking about?",
            "That's a question about timing for $topic. In teaching and assessment, we follow established schedules and developmental milestones. I can help you understand the right timing for different educational activities.",
            "For $topic, the when depends on context. In educational settings, we align timing with learning objectives, student readiness, and assessment requirements. What aspect of timing interests you?",
        ];
     ...
     */
    private function generateWhereAnswer($message)
    {
        $topic = str_replace(['where is', 'where are', 'where was', 'where were'], '', $message);
        $topic = trim($topic, '?');

        $responses = [
            "Regarding $topic, location and context matter in education. These things typically occur in learning environments that support effective teaching and assessment. Would you like recommendations for optimal settings?",
            "About $topic - the location depends on the educational context. We consider classroom environments, online spaces, and real-world settings for learning. What type of location are you asking about?",
            "That's a question about where $topic happens. In education, we choose locations that enhance learning, safety, and assessment validity. I can help you think about the best environments for different activities.",
            "For $topic, the location should support learning goals. Educational settings range from traditional classrooms to modern digital spaces. What specific location guidance do you need?",
        ];
    ...
    /**
     * Generate answer for "How much/many..." questions
     */
    private function generateHowMuchAnswer($message)
    {
        $topic = str_replace(['how much', 'how many'], '', $message);
        $topic = trim($topic, '?');

        $responses = [
            "Regarding $topic, the quantity depends on various factors. In educational contexts, we consider quality over quantity, appropriate amounts for learning goals, and assessment requirements. What specific amount are you asking about?",
            "About $topic - the right amount varies by situation. In teaching and assessment, we focus on what's effective for student learning and measurable outcomes. Would you like guidance on determining appropriate quantities?",
            "That's a question about quantity for $topic. In education, we balance comprehensive coverage with manageable amounts for optimal learning. I can help you determine the right amount for your situation.",
            "For $topic, the amount should align with learning objectives. Educational best practices guide us on appropriate quantities for different activities. What quantity considerations are you facing?",
        ];
    ...
    /**
     * Generate answer for other types of questions
     */
    private function generateOtherQuestionAnswer($message)
    {
        $topic = $this->extractTopic($message);

        $responses = [
            "That's an interesting question about $topic! 🤔 As your assessment assistant, I can share insights from educational best practices. In teaching and assessment contexts, $topic involves careful consideration of standards, student needs, and effective strategies. Would you like specific guidance?",
            "Great question on $topic! 💭 In the world of education, this relates to important principles of learning, assessment, and professional development. I can provide practical advice and evidence-based approaches. What would help you most?",
            "Regarding $topic, it's a significant area in education. I can offer guidance based on established standards, research, and best practices. Whether it's about teaching methods, assessment techniques, or student development, I'm here to help. How can I assist you?",
            "Excellent question about $topic! 🎯 As an AI assistant for TP assessments, I can provide thoughtful guidance on educational topics. This relates to key aspects of teaching, learning, and evaluation. What specific information would be most useful?",
        ];
    ...
    /**
     * Extract topic from a question by removing question words
     */
    private function extractTopic($message)
    {
        $questionWords = ['what', 'why', 'how', 'who', 'when', 'where', 'which', 'whose', 'whom', 'is', 'are', 'was', 'were', 'do', 'does', 'did', 'can', 'could', 'should', 'would', 'will', 'shall'];
        ...
        foreach ($words as $word) {
            ...
            if (!in_array($cleanWord, $questionWords) && strlen($cleanWord) > 1) {
                $topicWords[] = $word;
            }
        }

        $topic = implode(' ', $topicWords);
        return trim($topic) ?: 'general topics';
    }
}
